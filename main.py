...
from helpers import apply_difficulty_settings, get_randomized_ball_speed, save_score, show_level_transition, reset_game_state
...
while True:
    ...
    # PĘTLA do wpisania imienia
    player_name = ask_player_name(screen, SCREEN_WIDTH, SCREEN_HEIGHT, select_sound)
    if player_name is None:
        continue  # wracamy do start menu

    (current_level_index, brick_width, brick_height, bricks,
    ...
    waiting_for_respawn, lasers, laser_enabled, laser_timer) = reset_game_state(level_keys, difficulty, paddle, SCREEN_WIDTH, SCREEN_HEIGHT, LEVELS, ball_sound)
    ...
    while True:
                ...
                if laser.rect.bottom < 0:
                    ...
                    for brick in bricks[:]:
                        if laser.rect.colliderect(brick.rect):
                            # Laser niszczy brick całkowicie (nie zostawia hits_remaining = 1)
        ...
        if lives <= 0:
            ...
            if result == 'restart':
                ...
                break  # wyjście z pętli GRY -> wraca do menu STARTU

        if level_cleared:
            result = show_congratulations_screen(score, screen, SCREEN_WIDTH, SCREEN_HEIGHT, select_sound)
            if result == 'next_level' and current_level_index + 1 < len(level_keys):
                current_level_index += 1
                brick_width = SCREEN_WIDTH // len(LEVELS[level_keys[current_level_index]][0])
                brick_height = 30
                show_level_transition(screen, SCREEN_WIDTH, SCREEN_HEIGHT, background_image, current_level_index + 1)
                ...
                pygame.time.set_timer(WIDEN_SHRINK_RESET, 0)
                pygame.time.set_timer(SPEED_RESET, 0)
            elif result == 'next_level' and current_level_index + 1 == len(level_keys):
                # ukończył WSZYSTKIE poziomy → pokaż ekran ukończenia gry
                result = show_game_complete_screen(score, screen, SCREEN_WIDTH, SCREEN_HEIGHT, select_sound)
